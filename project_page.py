...
# #     path = r"C:\Users\mayur\OneDrive\Desktop\Final\assets\projects\Animation_girl.json"
...
def projects():
    st.title("🚀 Projects & Publications")
    st.markdown("---")
    ...
    projects_list = [
        {
            "title": "AI Data Analyst & Report Generator",
            "short_desc": "Automates data analysis and report generation from CSV files with multi-agent AI.",
            "tools": ["LangChain", "LangGraph", "Streamlit", "Pandas", "WeasyPrint"],
            "app_link": "https://ai-report-generator-2-0.streamlit.app",
            "github_link": "https://github.com/archanags001/AI-Report-Generator",
            "category": "AI Tools",
            "lottie": "assets/projects/ai_report.json",
            "image": "assets/projects/ai_report.png"
        },
        {
            "title": "Streamlit AI Guide",
            "short_desc": "Interactive AI tutor for learning Streamlit with conversational memory.",
            "tools": ["LangChain", "Streamlit", "Google Gemini API", "ChromaDB"],
            "app_link": "https://streamlit-ai-guide.streamlit.app",
            "github_link": "https://github.com/archanags001/streamlit-ai-guide",
            "category": "AI Tools",
            "lottie": "assets/projects/streamlit_ai.json",
            "image": "assets/projects/streamlit_ai.png"
        },
        {
            "title": "Insightful Data Explorer",
            "short_desc": "Comprehensive data exploration and AutoML with chat interface.",
            "tools": ["Python", "Streamlit", "PyCaret", "Google Gemini", "Pandas"],
            "app_link": "https://insightful-data-explorer-001.streamlit.app",
            "github_link": "https://github.com/archanags001/Insightful-Data-Explorer",
            "category": "Data Science",
            "lottie": "assets/projects/data_explorer.json",
            "image": "assets/projects/data_explorer.png"
        },
        # Add more projects here...
    ]

    # ----------------- Category Filter -----------------
    categories = ["All"] + sorted(list(set([p["category"] for p in projects_list])))
    category_filter = st.selectbox("Filter by Project Type:", categories)

    # ----------------- Filter Projects -----------------
    filtered_projects = [
        p for p in projects_list if category_filter == "All" or p["category"] == category_filter
    ]

    # ----------------- Display Projects in 2x2 Grid -----------------
    for i in range(0, len(filtered_projects), 2):
        ...
        for j, col in enumerate(cols):
            if i + j < len(filtered_projects):
                ...
                with col:
                    # Load Lottie animation if exists
                    lottie_file = project.get("lottie", "")
                    lottie_json = load_lottie_file(lottie_file) if lottie_file and os.path.exists(lottie_file) else None

                    if lottie_json:
                        st_lottie(lottie_json, height=150)
                    else:
                        # Fallback: load image if Lottie missing
                        img_file = project.get("image", "")
                        if img_file and os.path.exists(img_file):
                            st.image(img_file, use_column_width=True)
                        else:
                            st.write("🎬")  # Placeholder if both missing
                    ...
                    st.markdown(
                        ...
                        ">
                            ...
                            <p><b>Tools:</b> {', '.join(project['tools'])}</p>
                            <p>
                                {"<a href='"+project['app_link']+"' target='_blank'>App</a> | " if project['app_link'] else ""}
                                {"<a href='"+project['github_link']+"' target='_blank'>GitHub</a>" if project['github_link'] else ""}
                            </p>
                        </div>
                        """,
                        unsafe_allow_html=True
